...
export default function Contact() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Send form data to apurvasrikanakasabapathi@gmail.com via FormSubmit API
      const res = await fetch('https://formsubmit.co/ajax/apurvasrikanakasabapathi@gmail.com', {
        method: 'POST',
        headers: {
      ...
      if (res.ok) {
        ...
        const mailtoUrl = `mailto:apurvasrikanakasabapathi@gmail.com?subject=${encodeURIComponent(`Portfolio Message from ${formState.name}`)}&body=${encodeURIComponent(`Name: ${formState.name}\nEmail: ${formState.email}\n\nMessage:\n${formState.message}`)}`;
      ...
      <div className="w-full max-w-7xl mx-auto z-10 flex flex-col justify-between h-full">
        ...
        <div className="mt-20 pt-8 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-6 w-full">
          <span className="text-xs text-gray-500 font-mono">
            &copy; {new Date().getFullYear()} Apurvasri K. All rights reserved.
          </span>
          <div className="flex gap-6 text-gray-400">
            <a href="https://github.com/Apurvasri25" target="_blank" rel="noopener noreferrer" className="hover:text-neonBlue transition-colors flex items-center gap-1.5 text-xs font-mono">
              <AnimatedGithub className="w-4 h-4" /> Github
            </a>
            <a href="https://www.linkedin.com/in/apurvasri-k-921876293" target="_blank" rel="noopener noreferrer" className="hover:text-neonPurple transition-colors flex items-center gap-1.5 text-xs font-mono">
              <AnimatedLinkedin className="w-4 h-4" /> LinkedIn
            </a>
            <a href="mailto:apurvasrikanakasabapathi@gmail.com" className="hover:text-neonPink transition-colors flex items-center gap-1.5 text-xs font-mono">
